...
export APIVERSION
export PROVIDER
export BOILERPLATE="$(cat hack/boilerplate.go.txt)"
...
${GOMPLATE} < "hack/helpers/controller/KIND_LOWER/reconciler.go.tmpl" > "internal/controller/${kind_lower}/reconciler.go"
${GOMPLATE} < "hack/helpers/controller/KIND_LOWER/reconciler_test.go.tmpl" > "internal/controller/${kind_lower}/reconciler_test.go"

mkdir -p "internal/clients/${provider_lower}/${kind_lower}"
${GOMPLATE} < "hack/helpers/clients/KIND_LOWER/KIND_LOWER_client.go.tmpl" > "internal/clients/${provider_lower}/${kind_lower}/${kind_lower}_client.go"
${GOMPLATE} < "hack/helpers/clients/KIND_LOWER/KIND_LOWER_client_test.go.tmpl" > "internal/clients/${provider_lower}/${kind_lower}/${kind_lower}_client_test.go"

export DIR_LIST=$(find "internal/clients/${provider_lower}" -maxdepth 1 -type d | tail -n +2 | xargs -n1 basename | tr '\n' ' ')
${GOMPLATE} < "hack/helpers/controller/PROVIDER_LOWER.go.tmpl" > "internal/controller/${provider_lower}.go"