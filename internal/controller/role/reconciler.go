...
func (c *connector) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	c.log.Debug("Connecting to role resource", "name", cr.Name)
...
func (c *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	c.log.Debug("Observing role resource", "name", cr.Name)
	...
	if err != nil {
		c.log.Debug("Error observing role", "name", cr.Name, "error", err)
...
func (c *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	c.log.Debug("Creating role with parameters",
		"roleName", parameters.RoleName,
	...
	if err != nil {
		c.log.Debug("Error creating role", "name", cr.Name, "error", err)
...
func (c *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	...
	if !equalArrays(observedLdapGroups, desiredLdapGroups) {
		...
		if err != nil {
			c.log.Debug("Error updating role LDAP groups", "name", cr.Name, "error", err)
	...
	if !equalArrays(observedPrivileges, desiredPrivileges) {
		...
		c.log.Debug("Updating role privileges",
			"name", cr.Name,
			"roleName", parameters.RoleName,
		...
		if err != nil {
			c.log.Debug("Error updating role privileges", "name", cr.Name, "error", err)
			return managed.ExternalUpdate{}, fmt.Errorf(errUpdateRole, err)
		}
		cr.Status.AtProvider.Privileges = parameters.Privileges
