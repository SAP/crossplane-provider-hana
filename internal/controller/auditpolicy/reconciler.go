...
func (c *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	observed.AuditActions = auditpolicy.OptimizeAuditActions(observed.AuditActions)
	if len(observed.AuditActions) > 1 {
		return managed.ExternalObservation{}, errors.New("more than one audit action observed")
	}
...
func (c *external) buildObservedParameters(cr *v1alpha1.AuditPolicy) (*v1alpha1.AuditPolicyObservation, error) {
	...
	observed.AuditActions = auditpolicy.OptimizeAuditActions(observed.AuditActions)
	if len(observed.AuditActions) > 1 {
		return nil, errors.New("more than one audit action observed")
	}
	return observed, nil
}

func (c *external) buildDesiredParameters(cr *v1alpha1.AuditPolicy) (*v1alpha1.AuditPolicyParameters, error) {
	auditActions := auditpolicy.OptimizeAuditActions(cr.Spec.ForProvider.AuditActions)
