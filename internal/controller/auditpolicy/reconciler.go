...
import (
	...
	"github.tools.sap/cloud-orchestration/crossplane-provider-hana/internal/clients/hana/auditpolicy"
...
func (c *connector) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	c.log.Debug("Connecting to auditpolicy resource", "name", cr.Name)

	if err := c.db.Connect(ctx, s.Data); err != nil {
		c.log.Debug("Error connecting to hana in auditpolicy", "name", cr.Name, "error", err)
...
func (c *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	c.log.Debug("Observing auditpolicy resource", "name", cr.Name)
	...
	if err != nil {
		c.log.Debug("Error observing auditpolicy", "name", cr.Name, "error", err)
...
func (c *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	c.log.Debug("Creating auditPolicy with parameters",
	...
	if err != nil {
		c.log.Debug("Error creating auditpolicy", "name", cr.Name, "error", err)
...
func (c *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	...
	if (!equalArrays(observed.AuditActions, desired.AuditActions)) || (observed.AuditStatus != desired.AuditStatus) || (observed.AuditLevel != desired.AuditLevel) {
		...
		if err != nil {
			c.log.Debug("Error updating audit policy", "name", cr.Name, "error", err)
		...
		if *observed.AuditTrailRetention != *desired.AuditTrailRetention {
			c.log.Debug("Audit policy retention differ and will be updated",
				"name", cr.Name,
				"policyName", desired.PolicyName,
		...
		if *observed.Enabled != *desired.Enabled {
			c.log.Debug("Audit policy active state differ and will be updated",
				"name", cr.Name,
				"policyName", desired.PolicyName,
...
func (c *external) Delete(ctx context.Context, mg resource.Managed) (managed.ExternalDelete, error) {
	...
	if err != nil {
		c.log.Debug("Error deleting auditpolicy", "name", cr.Name, "error", err)
